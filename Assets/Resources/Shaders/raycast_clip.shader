...
Shader "Custom/Raycast Clip" {
	...
	SubShader {
		...
		Pass {
			...
			float _Brightness;
			float4 _ClipDimMin, _ClipDimMax, _ClipPlane;
			...
			float4 clip_data(float3 pos) {

				float4 data = tex3Dlod(_Data, float4(pos,0));

				float clipdata = step(_ClipDimMin.x, pos.x);
				clipdata *= step(_ClipDimMin.y, pos.y);
				clipdata *= step(_ClipDimMin.z, pos.z);
				clipdata *= step(pos.x, _ClipDimMax.x);
				clipdata *= step(pos.y, _ClipDimMax.y);
				clipdata *= step(pos.z, _ClipDimMax.z);
				clipdata *= step(0, dot(_ClipPlane, float4(pos - 0.5, 1)) + _ClipPlane.w);
	
				data.a *= saturate(clipdata);

				float4 col = float4(data.rgb, data.a);
				return col;
			}
			...
			float4 get_data(float3 pos) {
				...
				data *= step(0, dot(_ClipPlane, float4(pos - 0.5, 1)) + _ClipPlane.w);
...
#define STEP_CNT 512 
			...
			float4 frag(frag_input i) : COLOR
			{
				...
				{
					float4 voxel_col = clip_data(r_pos);
#ifdef FRONT_TO_BACK
